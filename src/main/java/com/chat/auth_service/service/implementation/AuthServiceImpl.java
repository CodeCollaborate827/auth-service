...
public class AuthServiceImpl implements AuthService {
    ...
    @Override
    public Mono<ResponseEntity<Login200Response>> login(Mono<LoginRequest> loginRequest, String requestId) {
        ...
        return refreshTokenRequest.flatMap(
                request ->
                        applicationTokenRepository
                                ...
                                .map(accessToken -> ResponseEntity.ok(mapRefreshTokenResponse(accessToken, requestId))));
    }
                            ...
                            .map(
                                    verificationCode ->
                                            Utils.createCommonSuccessResponse(
                                                    "Verification code is sent to your email. Please check your email to verify your account.", requestId));
                });
    }
        ...
        return resetPasswordRequest.flatMap(
                request -> {
                    return applicationTokenRepository
                            ...
                            .switchIfEmpty(Mono.error(new ApplicationException(ErrorCode.AUTH_ERROR19, requestId)))
                            .flatMap(
                                    token -> {
                            ...
                            .map(savedToken -> Utils.createCommonSuccessResponse("Reset password successfully", requestId));
                });
    }
    ...
    @Override
    public Mono<ResponseEntity<CommonResponse>> register(Mono<RegisterRequest> registerRequest, String requestId) {
                ...
                request -> {
                    ...
                    return createNewUserFromRequest(request, requestId)
                            ...
                            .map(
                                    savedUser ->
                                            Utils.createCommonSuccessResponse(
                                                    "Registration successful. Please check your email to verify your account.", requestId));
                });
    }

    private Mono<User> validateAndSaveLoginHistoryRequest(LoginRequest request, User user, String requestId) {
