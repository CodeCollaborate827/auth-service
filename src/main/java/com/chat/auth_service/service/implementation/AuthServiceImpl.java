...
public class AuthServiceImpl implements AuthService {
    ...
    public Mono<ResponseEntity<Login200Response>> login(Mono<LoginRequest> loginRequest) {
        return loginRequest.flatMap(request -> Mono.zip(userRepository.findByEmail(request.getEmail()), authenticationSettingRepository.findByUserId(null)) // TODO: we need to discuss this
                .flatMap(tuple -> {
                    User user = tuple.getT1();
                    AuthenticationSetting authSettings = tuple.getT2();

                    // Check if user exists
                    Mono.just(user).switchIfEmpty(Mono.error(new ApplicationException(ErrorCode.AUTH_ERROR1)));

                    // Check if password is correct
                    if (!passwordEncoder.matches(request.getPassword(), user.getPasswordHash())) {
                        return saveFailedLoginHistory(user.getId(), request).then(Mono.error(new ApplicationException(ErrorCode.AUTH_ERROR5)));
                    }

                    // Check if account is verified
                    if (user.getAccountStatus() == User.AccountStatus.UNVERIFIED) {
                        return saveFailedLoginHistory(user.getId(), request).then(Mono.error(new ApplicationException(ErrorCode.AUTH_ERROR3)));
                    }

                    // find all login history for the user and check ip_address and user_agent is
                    // matching or not
                    return loginHistoryRepository.findAllByUserId(user.getId()).collectList().flatMap(loginHistories -> {
                        LoginHistory loginHistory = loginHistories.stream().filter(history -> history.getIpAddress().equals(request.getIpAddress()) && history.getUserAgent().equals(request.getUserAgent())).findFirst().orElse(null);
                        ...
                        return saveRefreshToken(user, loginHistory).map(refreshToken -> ResponseEntity.ok(new Login200Response().refreshToken(refreshToken.getRefreshToken()).accessToken(accessToken)));
                    });
                }));
    }
    ...
    public Mono<ResponseEntity<Register200Response>> register(Mono<RegisterRequest> registerRequest) {
        return registerRequest.flatMap(request -> {
            ...
            return userRepository.findByEmail(request.getEmail()).flatMap(existingUser -> Mono.error(new ApplicationException(ErrorCode.AUTH_ERROR6))).then(Mono.defer(() -> userRepository.findByUsername(request.getUsername()))).flatMap(existingUser -> Mono.error(new ApplicationException(ErrorCode.AUTH_ERROR4)).then(Mono.defer(() -> {
                // Create user
                ...
                return Mono.zip(userRepository.save(newUser), authenticationSettingRepository.save(authSetting), verificationCodeRepository.save(verificationCode)).flatMap(tuple -> {
                    User savedUser = tuple.getT1();
                    VerificationCode savedCode = tuple.getT3();
                    // Send verification

                    mailUtils.sendVerificationEmail("Verify email", savedUser.getEmail(), savedCode);
    ...
    @Override
    public Mono<ResponseEntity<CommonResponse>> rendSendVerificationEmail(Mono<ResendVerificationEmailRequest> resendVerificationEmailRequest) {
        ...
        return resendVerificationEmailRequest.flatMap(request -> {
            return userRepository.findByEmail(request.getEmail())
                    ...
                    .flatMap(user -> {
                        return verificationCodeRepository.findByUserIdAndTypeLatest(user.getId(), VerificationCode.Type.valueOf(request.getType())).flatMap(verificationCode -> {
    ...
    @Override
    public Mono<ResponseEntity<VerifyEmailResponse>> verifyEmail(Mono<VerifyEmailRequest> verifyEmailRequest) {
        return verifyEmailRequest.flatMap(request -> {
            return getUserByEmailAndThrowExceptionNotExists(request.getEmail()).flatMap(user -> {
                ErrorCode errorCode = validateResendEmailRequest(user, request.getType());
    ...
    public Mono<ResponseEntity<RefreshTokenResponse>> refreshToken(Mono<RefreshTokenRequest> refreshTokenRequest) {
        return refreshTokenRequest.flatMap(request -> refreshTokenRepository  // find the refresh token in the database
                .findByRefreshToken(request.getRefreshToken()).switchIfEmpty(Mono.error(new ApplicationException(ErrorCode.AUTH_ERROR16))).flatMap(refreshToken -> loginHistoryRepository.findById(refreshToken.getLoginId()) // find the login history in the database
                        .flatMap(loginHistory -> {
                            ...
                            return userRepository.findById(loginHistory.getUserId()).flatMap(user -> {
                                ...
                                return refreshTokenRepository.save(refreshToken).map(savedRefreshToken -> {
    ...
    private Mono<VerifyEmailResponse> handleVerifyEmailForRegistration(VerifyEmailRequest request, User user) {
        return verificationCodeRepository.findByUserEmailAndCodeAndTypeLatest(request.getEmail(), request.getVerificationCode(), ACCOUNT_REGISTRATION).switchIfEmpty(Mono.error(new ApplicationException(ErrorCode.AUTH_ERROR12))).flatMap(verificationCode -> {
            ErrorCode errorCode = validateVerificationCode(verificationCode, request.getVerificationCode());
            ...
            Map<String, String> tokenMap = Map.of(ACCESS_TOKEN_KEY, accessToken, REFRESH_TOKEN_KEY, refreshToken);
    ...
    private Mono<VerifyEmailResponse> handleVerifyEmailForForgotPassword(VerifyEmailRequest request, User user) {
        return verificationCodeRepository.findByUserEmailAndCodeAndTypeLatest(request.getEmail(), request.getVerificationCode(), FORGOT_PASSWORD).switchIfEmpty(Mono.error(new ApplicationException(ErrorCode.AUTH_ERROR12))).flatMap(verificationCode -> {
            ErrorCode errorCode = validateVerificationCode(verificationCode, request.getVerificationCode());
            if (errorCode != null) {
                return Mono.error(new ApplicationException(errorCode));
            }
    ...
    private ErrorCode validateVerificationCode(VerificationCode verificationCodeInDB, String codeSentByUSer) {
    ...
    private Mono<User> getUserByEmailAndThrowExceptionNotExists(String email) {
        return userRepository.findByEmail(email).switchIfEmpty(Mono.error(new ApplicationException(ErrorCode.AUTH_ERROR1)));
    }
    ...
    private ErrorCode validateResendEmailRequest(User user, String emailVerificationType) {
        ...
        boolean isValidRequestType = Objects.nonNull(emailVerificationType) && Arrays.stream(values()).anyMatch(type -> type.name().equals(emailVerificationType.toUpperCase()));
