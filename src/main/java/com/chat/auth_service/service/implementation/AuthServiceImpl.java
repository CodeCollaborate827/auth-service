...
public class AuthServiceImpl implements AuthService {
        ...
        return forgotPasswordRequest.flatMap(
                request -> {
                    return userRepository
                            ...
                            .map(
                                    verificationCode -> Utils.createCommonSuccessResponse(
        ...
        return resetPasswordRequest.flatMap(
                request -> {
                    return applicationTokenRepository
                            ...
                            .map(
                                    savedToken -> Utils.createCommonSuccessResponse("Reset password successfully"));
                });
    }
        ...
        return registerRequest.flatMap(
                request -> {
                    ...
                    return createNewUserFromRequest(request)
                            ...
                            .map(
                                    savedUser -> Utils.createCommonSuccessResponse(
    ...
    private Mono<LoginHistory> validateAndSaveLoginHistoryRequest(LoginRequest request, User user) {
        ...
        return authenticationSettingRepository
                ...
                .zipWith(loginHistoryRepository.findByUserIdAndIpAddressAndUserAgent(user.getId(), request.getIpAddress(), request.getUserAgent()))
                .flatMap(tuple2 -> {
                    AuthenticationSetting authSettings = tuple2.getT1();
                    LoginHistory loginHistory = tuple2.getT2();

                    if (loginHistory == null) {
                        loginHistory = createNewLoginHistory(user, request);
                    }

                    if (!loginHistory.getIsSuccessful() && authSettings.getMfaEnabled()) {
                        LoginHistory finalLoginHistory = loginHistory;
                        return handleMfaLogin(user, request)
                                .flatMap(history -> {
                                    finalLoginHistory.setIsSuccessful(true);
                                    return loginHistoryRepository.save(finalLoginHistory);
                                });
                    }
